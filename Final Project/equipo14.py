...
class JugadorCaballosBailadoresEquipo14(JugadorCaballosBailadores):
    ...
    def heuristica(self, posicion):
        ...
        if self.triunfo(self._jugador_contrario(), posicion):
            distance_king += 100

        movements = len(self.posiciones_siguientes(posicion))
        value = 5 * movements - 3 * distance_king 
    ...
    def minimax(self, tree, maximizing_player, alpha=float('-inf'), beta=float('inf')):
        if len(tree) == 1:
            return tree[0]
